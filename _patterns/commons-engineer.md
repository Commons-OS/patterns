...
# ═══════════════════════════════════════════════════════════════════
context_labels:
  corporate: "Intrapreneurial Leadership Development Program"
  government: "Public Innovation & Systems Leadership Curriculum"
  activist: "Movement Builder & Systems Organizer Training"
  tech: "Platform Architect & Ecosystem Designer Pathway"
  community: "Community Steward & Commons Builder Development"

# ═══════════════════════════════════════════════════════════════════
# GROUP 3: ONTOLOGY & SEARCH OPTIMIZATION
# ═══════════════════════════════════════════════════════════════════
ontology:
  domain: meta
  cross_domains:
  ...
  search_hints:
    primary_tension: "Individual Capability vs. Collective Impact"
    vector_keywords:
      - commons engineering
      - systems thinking
  ...
  commons_assessment:
    ...
    vitality: 4.8
    vitality_reasoning: >-
      The Commons Engineer collection is designed to cultivate the very people who create vitality
      in living systems. It scores at the generative level because its purpose is not merely to
      sustain existing commons but to develop the practitioners who design, build, and steward new
      ones. The 7Cs arc — from inner clarity to systemic commons — mirrors the developmental
      journey of a living system itself, making the collection a fractal expression of the
...
relationships:
  ...
  specializes_to:
    ...
    - lighthouse-initiation
    - vitality-diagnosis
    - generative-exit
  enables:
    - {id: commons-blueprint, weight: 0.9, description: "The Commons Engineer collection develops the practitioners who apply the Commons Blueprint to real-world systems."}
  requires: []
  alternatives: []
  complementary:
    - {id: commons-blueprint, weight: 1.0, description: "The Blueprint defines what to build; the Engineer defines who builds it and how they develop. Together they form the foundation of the Commons Engineering curriculum."}
  tools: []
...
graph_garden:
  ...
  inferred_links:
    - target: commons-blueprint
      reason: "Complementary collection — Blueprint defines the system, Engineer defines the practitioner"
      strength: 5
...
> The commons will not build itself. It needs engineers — not of machines, but of living systems. This collection develops the person who can see, design, build, and steward the commons.
...
1. **Self-Directed Journey.** A practitioner can enter the collection at any point, drawn by the pattern that speaks to their current challenge. However, the 7Cs arc provides a recommended sequence. Begin with C1 (Clarity) and work forward. Each capability builds on the previous ones, and the patterns within each capability are designed to reinforce each other. A practitioner might spend weeks or months with a single capability before feeling ready to move to the next.

2. **Structured Curriculum.** The collection is designed to serve as the foundation for the Commons Engineering curriculum, alongside the Commons Blueprint collection. The two collections are complementary: the Blueprint defines *what* to build (the living system), while the Engineer defines *who* builds it and *how they develop*. A curriculum built on both collections would alternate between personal development (Engineer patterns) and systems design (Blueprint patterns), creating a rhythm of inner growth and outer application.

3. **Diagnostic Tool.** A practitioner or team can use the 7Cs framework as a self-assessment. For each capability, rate your current development on a scale of 1 to 5. Where are you strong? Where are the gaps? The patterns in each capability provide specific practices for addressing those gaps. This diagnostic can be repeated quarterly to track development over time.

4. **Community Practice.** The collection is most powerful when used in community. Form a cohort of aspiring Commons Engineers. Work through the patterns together. Share your experiences with each pattern — what worked, what didn't, what surprised you. Use the `pattern-sharing-practice` and `mutual-accountability` patterns to structure this collective learning. The community becomes both the context for learning and a living example of what you are learning to build.

5. **Lighthouse Application.** As practitioners progress through the arc, they should begin applying their developing capabilities to real-world projects — Lighthouses. A Lighthouse is a concrete instantiation of the commons in a specific domain. The `lighthouse-initiation` pattern provides the framework. The earlier capabilities (Clarity, Credibility, Capital) prepare the practitioner to contribute. The later capabilities (Character, Collaboration, Community, Commons) prepare them to lead.
...
The primary risk is premature advancement. A practitioner who rushes to C7 (Commons) without having genuinely developed C1 through C6 will build systems that reflect their underdevelopment — systems that are clever but lack integrity, that are technically sound but relationally fragile, that are well-designed but cannot sustain themselves. The arc is developmental for a reason: each capability provides the foundation for the next. Skipping stages creates structural weakness that will eventually manifest as systemic failure.
...
The open-source software movement provides a large-scale example of practitioner development in service of a commons. The most effective open-source contributors — people like Linus Torvalds, Vitalik Buterin, or Moxie Marlinspike — developed along a trajectory remarkably similar to the 7Cs. They began with deep technical clarity, built credibility through public contributions, created capital through reputation and intellectual assets, demonstrated character through principled leadership, collaborated across organizational boundaries, cultivated communities of practice, and ultimately designed systems that function as digital commons. None of them followed a formal curriculum. The Commons Engineer collection makes this implicit developmental path explicit and accessible.
...
AI transforms the developmental journey in specific ways. In C1 (Clarity), AI can serve as a perception amplifier — helping practitioners see patterns in data that would be invisible to human observation alone. In C2 (Credibility), AI dramatically lowers the cost of publishing and documentation, making it easier to build a visible body of work. In C3 (Capital), AI enables new forms of knowledge productization — turning a practitioner's expertise into scalable tools and services. In C4 (Character), AI cannot help — character is forged through lived experience, not algorithmic optimization. In C5 (Collaboration), AI can facilitate matching and coordination but cannot replace the relational depth that trust requires. In C6 (Community), AI can amplify collective sensing but must not replace the human practices that build belonging. In C7 (Commons), AI becomes a co-designer and co-operator of living systems, working alongside the Commons Engineer to build, monitor, and evolve the commons.
...
At the individual level, vitality looks like a person who is growing. They are not static — they are visibly developing new capabilities, deepening existing ones, and applying their learning to increasingly ambitious challenges. They publish regularly, collaborate generously, and take on leadership roles with both confidence and humility. They experience the joy of compounding — each year is richer than the last because it builds on the accumulated foundation of all previous years. They feel a sense of purpose and direction, not because their path is predetermined, but because they have developed the clarity to navigate uncertainty.

At the community level, vitality looks like a network that is alive. New practitioners are entering the arc, drawn by the visible body of work and the magnetic clarity of existing members. Experienced practitioners are mentoring newcomers, not out of obligation but out of genuine investment in the community's future. Patterns are being shared, adapted, and improved through collective practice. Lighthouses are being initiated, each one a living laboratory for the principles the community teaches. There is a sense of momentum — the feeling that something important is being built, and that every contribution matters.

Decay at the individual level looks like stagnation — a practitioner who has stopped growing, who recycles the same insights without deepening them, who has become comfortable in their current capability and resistant to the discomfort of the next developmental stage. Decay at the community level looks like insularity — a community that talks only to itself, that has become a club rather than a movement, that celebrates its own sophistication rather than its impact on the world. The antidote to both is the same: return to the living system. Go back to the territory. Initiate a Lighthouse. Apply the patterns to a real challenge. Let the friction of reality reveal where your development has gaps, and let the community hold you accountable for closing them.
